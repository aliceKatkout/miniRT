...
Test(camera, ray_pixel_center)
{
	...
	cr_expect(r.direction.x == 0);
	cr_expect(r.direction.y == 0);
	cr_expect(r.direction.z == -1);
}
...
Test(camera, ray_pixel_corner)
{
	...
	cr_expect(r.origin.y == 0);
	cr_expect(r.origin.z == 0);
	cr_expect(r.direction.x == 0.66519);
	cr_expect(r.direction.y == 0.33259);
	cr_expect(r.direction.z == -0.66851);
}
...
Test(camera, ray_pixel_transformed)
{
	...
	cr_expect(r.direction.x == sqrt(2) / 2);
	cr_expect(r.direction.y == 0);
	cr_expect(r.direction.z == -sqrt(2) / 2);
}